...
function VolumenSumergido() {
    if (dinoPosY >= nivelDelMar) {
        ...
        return (
            dino.clientWidth * Math.min(nivelDelMar - dinoPosY, dino.clientHeight)
        );
    }
}
...
function CrearMoneda() {
    ...
    moneda.style.bottom =
        monedaMinY + (monedaMaxY - monedaMinY) * Math.random() + "px";

    interactuables.push(moneda);
    tiempoHastaMoneda =
        tiempoMonedaMin +
        (Math.random() * (tiempoMonedaMax - tiempoMonedaMin)) / gameVel;
}
